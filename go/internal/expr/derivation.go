...
const (
	...
	// the tools that need it (kbuild's objtool, and its `ld -r` for
	// single-object modules) only appear in builds that already require
	// the sandbox.
	//
...
type Derivation struct {
	...
	// (kbuild compiles objtool during `make prepare`) and reaches the
