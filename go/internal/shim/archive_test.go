...
func TestParseARArgs(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			// built-in.a lists its subdirectories' built-in.a files
...
// The kernel case: every built-in.a is `ar cDPrST`, and kbuild nests
// archives so a directory's built-in.a lists its subdirectories'
// built-in.a alongside its own objects. Both used to fall through to
// passthrough, where the real ar met a drvref stub.
func TestParseARArgsKbuild(t *testing.T) {
	mods, archive, inputs, ok := parseARArgs([]string{
		"cDPrST", "lib/math/built-in.a",
		"lib/math/div64.o", "lib/math/gcd.o",
		"lib/math/tests/built-in.a",
	})
	if !ok {
		t.Fatalf("parseARArgs rejected a kbuild built-in.a invocation")
	}
	if mods != "cDPrST" {
		t.Errorf("modifiers = %q, want %q", mods, "cDPrST")
	}
	if archive != "lib/math/built-in.a" {
		t.Errorf("archive = %q", archive)
	}
	if len(inputs) != 3 || inputs[2] != "lib/math/tests/built-in.a" {
