...
func Compile(tool dispatch.Tool, args []string, cfg *toolchain.Config, l paths.Layout) error {
	...
	// string the preprocessor never looks at:
	//
	//	asm ("  .incbin \"kernel/config_data.gz\"  \n");
	//
	// `gcc -M` reports #include dependencies, so it cannot see this. The
	// file is missing from the staged tree and the TU dies in the
	// assembler rather than the compiler:
	//
	//	/build/ccaMdNtQ.s:11: Error: file not found: kernel/config_data.gz
	//
	// Nearly always the named file is GENERATED (config_data.gz,
	// kheaders_data.tar.xz, initramfs_data.cpio), so it is not a static
	// dependency we could teach the scanner about either.
	//
	// Detect the directive instead of listing paths. Enumerating them is
	// what the directory carveouts in mode.For already do by accident:
	// arch/x86/realmode, arch/x86/purgatory and efi/libstub are all
	// .incbin users that were carved out for OTHER observed symptoms.
	// Keying on the cause covers the rest — kernel/configs.c,
	// kernel/kheaders.c, certs/*.S, lib/bootconfig-data.S,
	// usr/initramfs_data.S — and every architecture, without a list to
	// maintain.
	...
	// 1b. If the caller asked the compiler for a dependency file, write
	// it ourselves. We stripped the flags that would have produced it
	// (they name paths relative to make's cwd, which the derivation's
	// read-only $src cannot be), so nothing else will — and some build
	// systems run a separate tool over the result and hard-fail without
	// it. kbuild pipes every single object through `fixdep`, which
	// exits with "error opening file: ./.<obj>.o.d".
	//
	// scan already resolved the exact header set, so this is not a
	// degraded substitute for the compiler's own output; for our
	// purposes it is the same information.
...
//
// Deliberately the plain `target: prereq…` shape rather than anything
// gcc-specific: consumers of this file (make's own include, kbuild's
// fixdep) only parse the target and its prerequisite list. `-MP`-style
// phony targets are not emitted — they exist to tolerate deleted
// headers, and a stale entry here costs at most one extra rebuild.
