...
// modelled. kbuild reaches it twice per module:
//
...
// (vmlinux, .so) go through the compiler driver in every build nixgg
...
func parseLDArgs(args []string) (flags []string, output string, inputs []string, ok bool) {
	...
	for i := 0; i < len(args); i++ {
		...
		case ldTwoArg[a]:
			...
			// value reads as a positional: kbuild's KBUILD_LDFLAGS
...
//
// NIXGG_REAL_LD wins, and the kernel needs it: nixpkgs deliberately
// passes the UNWRAPPED binutils ld there (see common-flags.nix — "The
// wrapper for ld.lld breaks linking the kernel"), which is not the
