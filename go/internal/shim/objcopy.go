...
//
// kbuild's generic object rewrite (scripts/Makefile.lib's
// `cmd_objcopy = $(OBJCOPY) $(OBJCOPYFLAGS) $< $@`) reads an object and
// writes another. Two places in an x86_64 build reach it before
// anything else does — arch/x86/boot/startup/ and
// drivers/firmware/efi/libstub/, both prefixing symbols — and both met
// a drvref stub:
//
//	objcopy: arch/x86/boot/startup/gdt_idt.o: file format not recognized
//
// Same derivation kind as objtool, different operand shape: objtool
// rewrites one operand in place, objcopy reads one file and writes
// another, so nothing needs copying out of the store first.
//
// Unlike objtool, the binary is already a store path (nixpkgs passes
// OBJCOPY=<gcc-wrapper>/bin/objcopy), so no storeAddTool step.
